...
export function maxHeapify(A: number[], i: number, heapSize: number, memento: HeapMemento ,currentAlg:HeapPseudoCodeKeys,gap:number = 0): void {
    const left = (2 * i) + 1;
    const right = (2 * i) + 2;
    ...
    if (left < heapSize && A[left] > A[largest]) {
        memento.addSnapshot({line: 4+gap, name: currentAlg},A, left, ActionType.HIGHLIGHT_LIGHT,heapSize,roles)
        largest = left;
    } else {
    ...
    if (right < heapSize && A[right] > A[largest]) {
        memento.addSnapshot({line: 7+gap, name: currentAlg},A, right, ActionType.HIGHLIGHT_LIGHT,heapSize,roles)
        largest = right;
    }
    memento.addBlank({line: 8+gap, name: currentAlg}, A,heapSize,roles)
    if (largest !== i) {
        memento.addSnapshot({line: 9+gap, name: currentAlg},A, largest, ActionType.HIGHLIGHT_FULL,heapSize,roles);
        roles = [];
...
export function heapIncreaseKey(A: number[], i: number, key: number, memento: HeapMemento,currentAlg:HeapPseudoCodeKeys,gap:number = 0): void {
